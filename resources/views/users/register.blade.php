...
<div class="flex justify-center">
   ...
   <div class="w-full">
     <div class="max-w-lg mx-auto  bg-white dark:bg-gray-800 rounded-lg px-8 py-10 flex flex-col items-center">
        ...
        <form action="{{ route('user.create') }}" class="w-full flex flex-col gap-4" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex items-start flex-col justify-start">
                ...
                <input type="email" id="email" name="email"
                    class="w-full px-3 dark:text-gray-200 dark:bg-gray-900 py-2 rounded-md border border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="example@gmail.com">
                    @error('email')
                    <span class="text-xs text-red-500">* {{ $message }}</span>
                    @enderror
            </div>
            ...
            <div class="flex items-start flex-col justify-start">
                ...
                <input type="password" id="password" name="password"
                    class="w-full px-3 dark:text-gray-200 dark:bg-gray-900 py-2 rounded-md border border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="*********">
                    @error('password')
                    <span class="text-xs text-red-500">* {{ $message }}</span>
                    @enderror
            </div>
            ...
            <button type="submit"
                class="bg-red-800 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-md shadow-sm">Sign Up</button>
        </form>
